...
function CardLivraison({
  ...
  const [pubsCollection] = useState(collection(db, "publication"));
  ...
  const addMessageNotif = async () => {
    await addDoc(notificationsCollection, {
      messageForAdmin: `Félicitations!! Votre coach ${nomCoach} vient de valider votre tâche  sur ${title}`,
  ...
  const handleValidation = async () => {
    ...
    const docId = idDoc;
    const docRef = doc(pubsCollection, docId);
  ...
  const messageNotifRejectedTask = async () => {
    ...
    await addDoc(notificationsCollection, {
      messageForAdmin: `Votre coach ${nomCoach} vient de rejeter votre tâche  sur ${title}. Consigne: Tâche  à refaire`,
      timestamp: serverTimestamp(),
      newNotif: true,
      email: emailStudent,
    });
    ...
    >
      ...
      >
        ...
        {role && role.role === "Coach" ? (
          <Modal.Footer>
            {!isCompleted && valid === false ? (
              <>
                {!validation ? (
                  <Button
                    className="border-0 d-flex justify-content-center bg-warning pe-auto"
                    disabled
                  >
                  ...
                  <Button
                    className="border-0 d-flex justify-content-center bg-success"
              ...
              <Button className="bg-secondary border-0 pe-auto " disabled>
                <Icon.Check2Square className="fs-4 me-1" /> Validée
              </Button>
            )}
          </Modal.Footer>
        ) : (
